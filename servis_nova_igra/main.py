...
SERVIS_IGRAC_URL = "http://servis_igraci:8002/odigraj"
...
async def stream_game():
    ...
    while potez_broj < 9 and pobjednik is None:
        servis_url = SERVIS_IGRAC_URL
        simbol = "X" if potez_broj % 2 == 0 else "O"
        servis_naziv = "Igrač X" if simbol == "X" else "Igrač O"

        try:
            podaci = await async_retry_request(servis_url, {"polje": polje, "simbol": simbol})
            polje = podaci["polje"]
            strategija = podaci["strategija"]
            pobjednik = provjeri_pobjednika(polje)
            yield f'data: {json.dumps({"poruka":f"{servis_naziv} je odigrao", "polje": polje, "strategija":strategija})}\n\n'
